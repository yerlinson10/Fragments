...
async function init() {
  ...
  if (testMode) {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "No se pudo cargar la historia de prueba"
        );
      }
    ...
    const selectedStory =
      localStorage.getItem("selectedStory") || "fragments_original";
...
function updateUI() {
  ...
  // Inventory
  if (
    engine.config.settings.show_inventory &&
    engine.config.inventory?.enabled
  ) {
...
function updateStatsDisplay() {
  ...
  for (const [key, statConfig] of Object.entries(engine.config.stats)) {
    ...
    const percent =
      ((value - statConfig.min) / (statConfig.max - statConfig.min)) * 100;
...
function updateCharactersDisplay() {
  if (
    ...
    Object.keys(engine.config.characters).length === 0
  ) {
...
function updateInventoryDisplay() {
  ...
  if (engine.gameState.inventory.items.length === 0) {
    elements.inventoryItems.innerHTML =
      '<small style="opacity:0.6">Vacío</small>';
  } else {
    engine.gameState.inventory.items.forEach((item) => {
      ...
      if (itemConfig) {
        itemSpan.textContent = `${itemConfig.icon || "📦"} ${
...
function updateProgress() {
  const progress =
    (currentEventIndex / Math.max(1, availableEvents.length)) * 100;
...
function showStatChanges(effects) {
  ...
  if (
    effects.inventory?.items_added &&
    effects.inventory.items_added.length > 0
  ) {
  ...
  if (
    effects.inventory?.items_removed &&
    effects.inventory.items_removed.length > 0
  ) {
  ...
  if (changes.length > 0) {
    ...
    // Sonido
    const totalEffect = Object.values(effects.stats || {}).reduce(
      (sum, data) => sum + data.change,
      0
    );
...
function checkAchievements(effects) {
  ...
  // Logros automáticos
  if (
    engine.gameState.current_day === 2 &&
    !engine.gameState.achievements.first_day_complete?.unlocked
  ) {
    engine.gameState.achievements.first_day_complete.unlocked = true;
    showAchievementNotification(
      engine.gameState.achievements.first_day_complete
    );
  }
}
...
function checkForDayEnd() {
  ...
  const endDayEvent = engine.story.events.find(
    (e) =>
...
document.getElementById("selectorBtn").onclick = () => {
  if (
    confirm("¿Volver al selector de historias? Se guardará automáticamente.")
  ) {
    engine.autoSave();
  ...
  saves.forEach((save) => {
    list.innerHTML += `
      <div class="save-slot-item">
        Slot ${save.slot}: Día ${save.day} | ${new Date(
      save.saved_at
    ).toLocaleString("es")}
      </div>
    `;
