...
function renderAll() {
  ...
  if (currentStory && currentStory.config) {
    ...
    const fields = [
      ...
      {
        id: "startingTime",
        value: currentStory.config.starting_time || "morning",
      },
    ...
    if (autoSaveEl)
      autoSaveEl.checked = currentStory.config.auto_save !== false;

    const enableSoundEl = document.getElementById("enableSound");
    if (enableSoundEl)
      enableSoundEl.checked = currentStory.config.enable_sound !== false;

    const showCharactersEl = document.getElementById("showCharacters");
    if (showCharactersEl)
      showCharactersEl.checked = currentStory.config.show_characters || false;

    const showInventoryEl = document.getElementById("showInventory");
    if (showInventoryEl)
      showInventoryEl.checked = currentStory.config.show_inventory || false;

    // Inventario
    const inventoryEnabledEl = document.getElementById("inventoryEnabled");
    if (inventoryEnabledEl)
      inventoryEnabledEl.checked =
        currentStory.config.inventory?.enabled || false;

    const initialMoneyEl = document.getElementById("initialMoney");
    if (initialMoneyEl)
      initialMoneyEl.value = currentStory.config.inventory?.initial_money || 0;

    // Final por defecto
    const defaultEndingTitleEl = document.getElementById("defaultEndingTitle");
    if (defaultEndingTitleEl)
      defaultEndingTitleEl.value = currentStory.endings.default?.title || "";

    const defaultEndingMessageEl = document.getElementById(
      "defaultEndingMessage"
    );
    if (defaultEndingMessageEl)
      defaultEndingMessageEl.value =
        currentStory.endings.default?.message || "";
  }
...
document
  .getElementById("keyboardShortcutsBtn")
...
document
  .getElementById("statModalCancel")
  .addEventListener("click", () => closeModal("statModal"));
document
  .getElementById("flagModalCancel")
  .addEventListener("click", () => closeModal("flagModal"));
document
  .getElementById("characterModalCancel")
  .addEventListener("click", () => closeModal("characterModal"));
document
  .getElementById("itemModalCancel")
  .addEventListener("click", () => closeModal("itemModal"));
document
  .getElementById("endingModalCancel")
  .addEventListener("click", () => closeModal("endingModal"));
document
  .getElementById("achievementModalCancel")
  .addEventListener("click", () => closeModal("achievementModal"));
...
async function loadExistingStory(storyId) {
  try {
    ...
    if (
      ...
      !Array.isArray(currentStory.story.events)
    ) {
      throw new Error("Archivo story.json con estructura inválida");
    }

    if (
      !currentStory.endings.endings ||
      !Array.isArray(currentStory.endings.endings)
    ) {
...
function updateUI() {
  ...
  document.getElementById("storyVersion").value =
    currentStory.config.story.version || "1.0.0";
  document.getElementById("storyTitleInput").value =
    currentStory.config.story.title || "";
  document.getElementById("storySubtitle").value =
    currentStory.config.story.subtitle || "";
  document.getElementById("storyDescription").value =
    currentStory.config.story.description || "";
  document.getElementById("storyAuthor").value =
    currentStory.config.story.author || "";
  document.getElementById("storyDays").value =
    currentStory.config.story.max_days || currentStory.config.story.days || 1;
  document.getElementById("startingDay").value =
    currentStory.config.story.starting_day || 1;
  document.getElementById("startingTime").value =
    currentStory.config.story.starting_time || "morning";
  document.getElementById("saveSlots").value =
    currentStory.config.settings.save_slots || 3;
  document.getElementById("autoSave").checked =
    currentStory.config.settings.auto_save !== false;
  document.getElementById("enableSound").checked =
    currentStory.config.settings.enable_sound !== false;
  document.getElementById("showCharacters").checked =
    currentStory.config.settings.show_characters === true;
  document.getElementById("showInventory").checked =
    currentStory.config.settings.show_inventory === true;

  // Inventory
  document.getElementById("inventoryEnabled").checked =
    currentStory.config.inventory.enabled === true;
  document.getElementById("initialMoney").value =
    currentStory.config.inventory.money || 0;

  // Default ending
  document.getElementById("defaultEndingTitle").value =
    currentStory.endings.default_ending.title || "";
  document.getElementById("defaultEndingMessage").value =
    currentStory.endings.default_ending.content.message || "";

  // Update title
  document.getElementById("storyTitle").textContent =
    currentStory.config.story.title || "Nueva Historia";
    ...
    <div class="item-card">
      <div class="item-header">
        <div class="item-title">
          <i data-lucide="${stat.icon || "bar-chart-2"}"></i> ${
        stat.name || key
      }
        </div>
...
function renderFlags() {
  ...
  container.innerHTML = Object.entries(flags)
    .map(
      ([key, value]) => `
    <div class="item-card">
      <div class="item-header">
      ...
      <div class="item-content">
        <div class="form-group">
          <label>Valor inicial: <strong>${JSON.stringify(
            value
          )}</strong></label>
        </div>
        <div class="form-group">
...
window.addFlag = function () {
  ...
  document.getElementById("flagValueLabel").textContent =
...
window.addCharacter = function () {
  ...
  newSave.onclick = () => {
    ...
    const relationship = parseInt(
      document.getElementById("characterRelationship").value
    );
...
function renderItems() {
  ...
  if (Object.keys(items).length === 0) {
    container.innerHTML =
      '<div class="empty-message"><p>No hay items definidos</p></div>';
    return;
  }
    ...
    <div class="item-card">
      <div class="item-header">
        <div class="item-title">
          ...
          ${
            isInitial
              ? '<span class="badge-success" title="Se tiene desde el inicio">Inicial</span>'
              : ""
          }
        </div>
        <div class="item-actions">
...
window.editItem = function (key) {
  ...
  newSave.onclick = () => {
    ...
    const initialIndex =
      currentStory.config.inventory.initial_items.indexOf(key);
...
function renderEvents() {
  ...
  const eventsWithOriginalIndex = events.map((event, originalIndex) => ({
    event,
    originalIndex,
  }));
  ...
  if (eventsFilter !== "all") {
    filteredEvents = filteredEvents.filter(
      (e) => e.event.type === eventsFilter
    );
  }

  if (dayFilterValue !== "all") {
    filteredEvents = filteredEvents.filter(
      (e) => e.event.day === parseInt(dayFilterValue)
    );
  }
    ...
    <div class="event-card" draggable="true" data-index="${originalIndex}">
      <div class="event-header">
        ...
        <div style="display: flex; gap: 5px; align-items: center;">
          <span class="event-type-badge ${event.type || "optional"}">${
        event.type || "optional"
      }</span>
          ...
          <button class="btn-icon" onclick="moveEventUp(${originalIndex}); event.stopPropagation();" 
                  ${
                    isFirst ? "disabled" : ""
                  } title="Subir"><i data-lucide="arrow-up"></i></button>
          <button class="btn-icon" onclick="moveEventDown(${originalIndex}); event.stopPropagation();" 
                  ${
                    isLast ? "disabled" : ""
                  } title="Bajar"><i data-lucide="arrow-down"></i></button>
        </div>
      </div>
      <div class="event-situation" onclick="editEvent(${originalIndex})">${
        event.text ? event.text.substring(0, 150) : ""
      }${event.text && event.text.length > 150 ? "..." : ""}</div>
      <div class="event-meta" onclick="editEvent(${originalIndex})">
        <span><i data-lucide="calendar"></i> Día ${
          event.day || "cualquiera"
        }</span>
        <span><i data-lucide="shuffle"></i> ${
          event.choices?.length || 0
        } opciones</span>
        ${
          event.can_repeat
            ? '<span><i data-lucide="repeat"></i> Repetible</span>'
            : ""
        }
      </div>
      ${
        ...
        <div class="event-choices" onclick="editEvent(${originalIndex})">
          ...
          ${event.choices
            .map(
              (choice, i) => `
            <div class="choice-item">${i + 1}. ${choice.text.substring(0, 60)}${
    ...
    card.addEventListener("drop", (e) => {
      ...
      if (draggedElement !== card) {
        ...
        if (draggedIndex !== null && targetIndex !== null) {
          ...
          const [movedEvent] = currentStory.story.events.splice(
            draggedIndex,
            1
          );
...
window.editEvent = function (index) {
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <select id="eventType">
          <option value="optional" ${
            event.type === "optional" ? "selected" : ""
          }>Opcional</option>
          <option value="mandatory" ${
            event.type === "mandatory" ? "selected" : ""
          }>Obligatorio</option>
          <option value="random" ${
            event.type === "random" ? "selected" : ""
          }>Aleatorio</option>
          <option value="forced" ${
            event.type === "forced" ? "selected" : ""
          }>Forzado</option>
        </select>
      </div>
      
      <div class="form-group" id="probabilityGroup" style="${
        event.type === "random" ? "" : "display:none"
      }">
        <label>Probabilidad (0-1)</label>
        <input type="number" id="eventProbability" min="0" max="1" step="0.1" value="${
      ...
      <div class="form-group">
        ...
        <select id="eventTime">
          <option value="" ${
            !event.time ? "selected" : ""
          }>Cualquier momento</option>
          <option value="morning" ${
            event.time === "morning" ? "selected" : ""
          }>Mañana</option>
          <option value="afternoon" ${
            event.time === "afternoon" ? "selected" : ""
          }>Tarde</option>
          <option value="evening" ${
            event.time === "evening" ? "selected" : ""
          }>Noche</option>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="number" id="eventEarliestHour" min="0" max="23" value="${
          event.earliest_hour !== undefined ? event.earliest_hour : ""
        }" placeholder="Ej: 8" />
      </div>
      
      <div class="form-group">
        <label>Hora más tardía (0-23)</label>
        <input type="number" id="eventLatestHour" min="0" max="23" value="${
          event.latest_hour !== undefined ? event.latest_hour : ""
        }" placeholder="Ej: 22" />
      </div>
      
      <div class="form-group">
        <label class="checkbox-label">
          <input type="checkbox" id="eventCanRepeat" ${
            event.can_repeat ? "checked" : ""
          } />
          Puede repetirse
        </label>
      </div>
      ...
      <div class="form-group full-width">
        ...
        <textarea id="eventSituation" rows="4" required>${
          event.text || ""
        }</textarea>
      </div>
          ...
          <div class="conditions-builder">
            ...
            <div class="condition-section">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                <div>
                  ...
                  <input type="number" id="eventDayMin" min="0" value="${
                    event.conditions?.day_min || ""
                  }" placeholder="Desde día..." />
                </div>
                <div>
                  <label>Día máximo</label>
                  <input type="number" id="eventDayMax" min="0" value="${
                    event.conditions?.day_max || ""
                  }" placeholder="Hasta día..." />
                </div>
              </div>
            </div>
            ...
            <div class="condition-section">
              ...
              <div>
                ...
                <input type="number" id="eventMoneyMin" value="${
                  event.conditions?.inventory?.money_min || ""
                }" placeholder="Ej: 100" />
              </div>
  ...
  typeSelect.addEventListener("change", () => {
    probabilityGroup.style.display =
      typeSelect.value === "random" ? "" : "none";
  });
};
...
function loadEventConditions(conditions) {
    ...
    Object.entries(conditions.stats).forEach(([key, value]) => {
      ...
      // Formato editor antiguo: { "honor": { min: 50, max: 100 } }
        ...
        if (value.min !== undefined)
          addEventStatCondition(key, "min", value.min);
        if (value.max !== undefined)
          addEventStatCondition(key, "max", value.max);
      }
    });
  }
  ...
  if (conditions.characters) {
    Object.entries(conditions.characters).forEach(([char, values]) => {
      if (values.relationship_min !== undefined)
        addEventCharacterCondition(
          char,
          "relationship_min",
          values.relationship_min
        );
      if (values.relationship_max !== undefined)
        addEventCharacterCondition(
          char,
          "relationship_max",
          values.relationship_max
        );
      if (values.met !== undefined)
        addEventCharacterCondition(char, "met", values.met);
    });
  }
  ...
  if (conditions.previous_choices) {
    Object.entries(conditions.previous_choices).forEach(
      ([eventId, choiceIndex]) => {
...
function getEventConditions() {
  ...
  // completed_events
  if (
    eventConditionsData.completed_events &&
    eventConditionsData.completed_events.length > 0
  ) {
  ...
  // previous_choices
  if (
    eventConditionsData.previous_choices &&
    Object.keys(eventConditionsData.previous_choices).length > 0
  ) {
  ...
  if (
    moneyMin ||
    (eventConditionsData.inventory?.has_items &&
      eventConditionsData.inventory.has_items.length > 0)
  ) {
...
window.addEventStatCondition = function (
  statKey = "",
  type = "min",
  value = 0
) {
  ...
  div.innerHTML = `
    <select class="stat-select">
      ...
      ${availableStats
        .map(
          (s) =>
            `<option value="${s}" ${s === statKey ? "selected" : ""}>${
              currentStory.config.stats[s].name
            }</option>`
        )
        .join("")}
    </select>
  ...
  const update = () => {
    ...
    if (stat) {
      if (!eventConditionsData.stats[stat])
        eventConditionsData.stats[stat] = {};
...
window.removeEventStatCondition = function (id) {
  ...
  if (eventConditionsData.stats[stat]) {
    ...
    if (Object.keys(eventConditionsData.stats[stat]).length === 0)
      delete eventConditionsData.stats[stat];
  }
  div.remove();
};
...
window.addEventCharacterCondition = function (
  charKey = "",
  ...
  div.innerHTML = `
    <select class="char-select">
      ...
      ${availableChars
        .map(
          (c) =>
            `<option value="${c}" ${c === charKey ? "selected" : ""}>${
              currentStory.config.characters[c].name
            }</option>`
        )
        .join("")}
    </select>
    <select class="char-type">
      <option value="relationship_min" ${
        type === "relationship_min" ? "selected" : ""
      }>Relación mín</option>
      <option value="relationship_max" ${
        type === "relationship_max" ? "selected" : ""
      }>Relación máx</option>
  ...
  const update = () => {
    ...
    if (char) {
      ...
      if (!eventConditionsData.characters[char])
        eventConditionsData.characters[char] = {};
...
window.removeEventCharacterCondition = function (id) {
  ...
  if (eventConditionsData.characters[char]) {
    ...
    if (Object.keys(eventConditionsData.characters[char]).length === 0)
      delete eventConditionsData.characters[char];
  }
  div.remove();
};
...
window.addCompletedEventCondition = function (eventId = "") {
  ...
  const update = () => {
    ...
    if (event) {
      if (!eventConditionsData.completed_events)
        eventConditionsData.completed_events = [];
...
window.addPreviousChoiceCondition = function (eventId = "", choiceIndex = 0) {
  ...
  const availableEvents = currentStory.story.events.filter(
  ...
  const update = () => {
    ...
    if (event) {
      if (!eventConditionsData.previous_choices)
        eventConditionsData.previous_choices = {};
...
window.addRequiredItem = function (itemName = "") {
  ...
  const update = () => {
    const item =
      ...
      div.querySelector(".item-input")?.value;
    if (item) {
      if (!eventConditionsData.inventory) eventConditionsData.inventory = {};
      if (!eventConditionsData.inventory.has_items)
        eventConditionsData.inventory.has_items = [];
  ...
  const input =
    div.querySelector(".item-select") || div.querySelector(".item-input");
...
function renderEventChoices(choices) {
  ...
  container.innerHTML = choices
    ...
    <div class="item-card" style="margin-bottom: 1rem;">
      ...
      <div class="form-group full-width">
        ...
        <input type="text" class="choice-text" data-index="${i}" value="${
        choice.text
      }" />
      </div>
      <div class="form-group full-width">
        ...
        <div class="collapsible-content">
          <div class="conditions-builder" style="margin-top: 0.5rem;">
            ...
            <div class="condition-section">
              ...
              <label class="checkbox-label">
                <input type="checkbox" class="trigger-ending" data-index="${i}" ${
        choice.effects?.trigger_ending ? "checked" : ""
      } />
                Terminar juego (trigger_ending)
              </label>
              <label class="checkbox-label">
                <input type="checkbox" class="trigger-next-day" data-index="${i}" ${
        choice.effects?.trigger_next_day ? "checked" : ""
      } />
  ...
  choices.forEach((choice, i) => {
    ...
    if (choice.effects.unlock_events) {
      choice.effects.unlock_events.forEach((eventId) =>
        addUnlockEvent(i, eventId)
      );
    }
  ...
  container.querySelectorAll(".trigger-ending").forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      ...
      if (!checkbox.checked)
        delete eventChoicesData[idx].effects.trigger_ending;
    });
  });
  ...
  container.querySelectorAll(".trigger-next-day").forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      ...
      if (!checkbox.checked)
        delete eventChoicesData[idx].effects.trigger_next_day;
    });
  });
}
...
window.addChoiceStatEffect = function (choiceIndex, statKey = "", value = 0) {
  ...
  const update = () => {
    ...
    const val = parseInt(div.querySelector(".stat-value").value) || 0;
    if (stat) {
      if (!eventChoicesData[choiceIndex].effects.stats)
        eventChoicesData[choiceIndex].effects.stats = {};
...
window.addChoiceFlagEffect = function (
  choiceIndex,
  flagKey = "",
  value = true
) {
  ...
  const update = () => {
    ...
    if (flag) {
      ...
      if (!eventChoicesData[choiceIndex].effects.flags)
        eventChoicesData[choiceIndex].effects.flags = {};
...
window.addChoiceItemEffect = function (
  choiceIndex,
  itemKey = "",
  action = "add"
) {
  ...
  div.innerHTML = `
    <select class="item-select">
      ...
      ${availableItems
        .map(
          (item) =>
            `<option value="${item}" ${item === itemKey ? "selected" : ""}>${
              currentStory.config.inventory.items[item].name
            }</option>`
        )
        .join("")}
    </select>
  ...
  const update = () => {
    ...
    if (eventChoicesData[choiceIndex].effects.inventory?.[otherAction]) {
      const idx =
        eventChoicesData[choiceIndex].effects.inventory[otherAction].indexOf(
          item
        );
      if (idx > -1) {
        eventChoicesData[choiceIndex].effects.inventory[otherAction].splice(
          idx,
          1
        );
      }
    ...
    if (item) {
      ...
      if (
        !eventChoicesData[choiceIndex].effects.inventory[act].includes(item)
      ) {
...
window.removeChoiceItemEffect = function (id, choiceIndex) {
  ...
  if (
    eventChoicesData[choiceIndex].effects.inventory &&
    eventChoicesData[choiceIndex].effects.inventory[action]
  ) {
    const idx =
      eventChoicesData[choiceIndex].effects.inventory[action].indexOf(item);
    if (idx > -1)
      eventChoicesData[choiceIndex].effects.inventory[action].splice(idx, 1);
  }
  div.remove();
};

window.addChoiceCharacterEffect = function (
  choiceIndex,
  charKey = "",
  value = 0
) {
  ...
  const update = () => {
    const char = div.querySelector(".char-select").value;
    const relationship =
      parseInt(div.querySelector(".char-relationship").value) || 0;
...
window.addUnlockEvent = function (choiceIndex, eventId = "") {
  const container = document.getElementById(
    `choice-${choiceIndex}-unlock-events`
  );
  ...
  const update = () => {
    ...
    if (event) {
      ...
      const index =
        eventChoicesData[choiceIndex].effects.unlock_events.indexOf(event);
      if (index === -1) {
...
window.addLockEvent = function (choiceIndex, eventId = "") {
  const container = document.getElementById(
    `choice-${choiceIndex}-lock-events`
  );
  ...
  const update = () => {
    ...
    if (event) {
      ...
      const index =
        eventChoicesData[choiceIndex].effects.lock_events.indexOf(event);
      if (index === -1) {
...
window.addAchievementUnlock = function (choiceIndex, achievementKey = "") {
  const container = document.getElementById(
    `choice-${choiceIndex}-achievement`
  );
  const id = `choice-${choiceIndex}-ach-${Date.now()}`;
  const availableAchievements = Object.keys(
    currentStory.config.achievements || {}
  );
  ...
  div.innerHTML = `
    <select class="achievement-select">
      ...
      ${availableAchievements
        .map(
          (a) =>
            `<option value="${a}" ${a === achievementKey ? "selected" : ""}>${
              currentStory.config.achievements[a].name
            }</option>`
        )
        .join("")}
    </select>
...
window.removeAchievementUnlock = function (id, choiceIndex) {
  ...
  if (eventChoicesData[choiceIndex].effects.unlocks) {
    ...
    if (
      Object.keys(eventChoicesData[choiceIndex].effects.unlocks).length === 0
    ) {
...
function renderEndings() {
  ...
  container.innerHTML = endings
    .map(
      (ending, index) => `
    <div class="item-card">
      <div class="item-header">
        <div class="item-title"><i data-lucide="flag-triangle-right"></i> ${
          ending.id
        }</div>
        <div class="item-actions">
...
function loadEndingConditions(conditions) {
  ...
  if (conditions.inventory) {
    if (conditions.inventory.money_min) {
      document.getElementById("endingMoneyMin").value =
        conditions.inventory.money_min;
    }
    if (conditions.inventory.has_items) {
      conditions.inventory.has_items.forEach((item) => {
...
function getEndingConditions() {
  ...
  if (Object.keys(endingConditionsData.characters).length > 0) {
    conditions.characters = {};
    Object.entries(endingConditionsData.characters).forEach(
      ([char, values]) => {
        conditions.characters[char] = values;
      }
    );
  ...
  // completed_events
  if (
    endingConditionsData.completed_events &&
    endingConditionsData.completed_events.length > 0
  ) {
  ...
  // previous_choices
  if (
    endingConditionsData.previous_choices &&
    Object.keys(endingConditionsData.previous_choices).length > 0
  ) {
  ...
  if (
    moneyMin ||
    (endingConditionsData.inventory?.has_items &&
      endingConditionsData.inventory.has_items.length > 0)
  ) {
    conditions.inventory = {};
    if (moneyMin) conditions.inventory.money_min = moneyMin;
    if (
...
window.addEndingStatCondition = function (
  statKey = "",
  type = "min",
  value = 0
) {
...
window.addEndingCharacterCondition = function (
  charKey = "",
  type = "relationship_min",
  value = 0
) {
  ...
  div.innerHTML = `
    ...
    <select class="char-type" data-id="${id}">
      <option value="relationship_min" ${
        type === "relationship_min" ? "selected" : ""
      }>Relación mínima</option>
      <option value="relationship_max" ${
        type === "relationship_max" ? "selected" : ""
      }>Relación máxima</option>
      <option value="met" ${type === "met" ? "selected" : ""}>Conocido</option>
    </select>
...
window.addEndingCompletedEvent = function (eventId = "") {
  ...
  const update = () => {
    const event = div.querySelector(".event-select").value;
    if (event) {
      if (!endingConditionsData.completed_events)
        endingConditionsData.completed_events = [];
...
window.addEndingPreviousChoice = function (eventId = "", choiceIndex = 0) {
  ...
  const update = () => {
    ...
    const index = parseInt(div.querySelector(".choice-index").value) || 0;
    if (event) {
      if (!endingConditionsData.previous_choices)
        endingConditionsData.previous_choices = {};
...
window.addEndingRequiredItem = function (itemName = "") {
  ...
  const update = () => {
    ...
    if (item) {
      ...
      if (!endingConditionsData.inventory.has_items)
        endingConditionsData.inventory.has_items = [];
...
function renderAchievements() {
  ...
  container.innerHTML = Object.entries(achievements)
    .map(
      ([key, ach]) => `
    <div class="item-card">
      <div class="item-header">
...
window.refreshFlowchart = async function () {
  ...
  if (!currentStory.story.events || currentStory.story.events.length === 0) {
    ...
    document.getElementById("flowchartStats").innerHTML =
      "<p>No hay eventos para analizar.</p>";
    return;
  }
...
function addNodeClickHandlers() {
  ...
  nodes.forEach((node) => {
    ...
    const event = currentStory.story.events.find(
      (e) => sanitizeId(e.id) === nodeId
    );
    if (!event) return;
...
function showEventDetails(event) {
  ...
  const conditionsHtml =
    event.conditions && Object.keys(event.conditions).length > 0
      ? `<pre style="background: var(--bg-secondary); padding: 0.5rem; border-radius: 4px; overflow: auto; max-height: 200px;">${JSON.stringify(
          event.conditions,
          null,
          2
        )}</pre>`
  ...
  modal.innerHTML = `
    <div class="modal-content large">
      ...
      <div style="display: grid; gap: 1rem;">
        <div>
          <strong>Tipo:</strong> <span style="padding: 0.25rem 0.5rem; background: var(--accent); color: white; border-radius: 4px; font-size: 0.9rem;">${
            event.type || "optional"
          }</span>
        ...
        <div>
          ...
          <p style="padding: 0.5rem; background: var(--bg-secondary); border-radius: 4px; margin-top: 0.5rem;">${
      ...
      <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
        <button class="btn-primary" onclick="editEventFromFlowchart('${
          event.id
        }')"><i data-lucide="pencil"></i> Editar Evento</button>
...
window.editEventFromFlowchart = function (eventId) {
  ...
  setTimeout(() => {
    const eventIndex = currentStory.story.events.findIndex(
      (e) => e.id === eventId
    );
    if (eventIndex !== -1) {
...
function generateMermaidCode() {
  ...
  const dayFilter =
    document.getElementById("flowchartDayFilter")?.value || "all";
  const typeFilter =
    document.getElementById("flowchartTypeFilter")?.value || "all";
  const showUnreachable =
    document.getElementById("showUnreachable")?.checked !== false;
  const showOnlyMandatory =
    document.getElementById("showOnlyMandatory")?.checked || false;
  const searchText =
    document.getElementById("flowchartSearch")?.value.toLowerCase() || "";
  ...
  let filteredEvents = events.filter((event) => {
    ...
    if (searchText) {
      ...
      const matchText =
        event.text && event.text.toLowerCase().includes(searchText);
  ...
  if (!showUnreachable) {
    filteredEvents = filteredEvents.filter(
      (event) => reachable && reachable.has(event.id)
    );
  }
  ...
  mermaidCode +=
    "  classDef mandatory fill:#4a90e2,stroke:#2563eb,stroke-width:3px,color:#fff,font-weight:bold\n";
  mermaidCode +=
    "  classDef optional fill:#10b981,stroke:#059669,stroke-width:2px,color:#fff\n";
  mermaidCode +=
    "  classDef random fill:#f59e0b,stroke:#d97706,stroke-width:2px,color:#fff\n";
  mermaidCode +=
    "  classDef forced fill:#8b5cf6,stroke:#7c3aed,stroke-width:3px,color:#fff,font-weight:bold\n";
  mermaidCode +=
    "  classDef unreachable fill:#ef4444,stroke:#dc2626,stroke-width:2px,color:#fff,opacity:0.6\n";
  mermaidCode +=
    "  classDef dayLabel fill:#f3f4f6,stroke:#9ca3af,stroke-width:2px,color:#374151,font-weight:bold\n";
  ...
  sortedDays.forEach((day, dayIndex) => {
    ...
    dayEvents.forEach((event, index) => {
      ...
      const indicatorsText =
        indicators.length > 0
          ? `<br/><small>${indicators.join(" • ")}</small>`
          : "";
    ...
    events.forEach((targetEvent) => {
      if (targetEvent.conditions?.previous_choices) {
        if (targetEvent.conditions.previous_choices[event.id] !== undefined) {
          ...
          if (!processedEdges.has(edgeKey)) {
            mermaidCode += `  ${fromId} -->|🎯 Si elegiste ${
              choiceIndex + 1
            }| ${toId}\n`;
            processedEdges.add(edgeKey);
          }
    ...
    events.forEach((event) => {
      if (!reachable.has(event.id)) {
        if (event.conditions?.completed_events) {
          const allReachable = event.conditions.completed_events.every(
            (reqId) => reachable.has(reqId)
          );
          if (allReachable) {
            reachable.add(event.id);
            changed = true;
...
function updateFlowchartStats() {
  ...
  const stats = {
    ...
    withConditions: events.filter(
      (e) => e.conditions && Object.keys(e.conditions).length > 0
    ).length,
  ...
  const criticalPathText =
    criticalPath.length > 0
      ? `${criticalPath.length} eventos (${criticalPath
          .map((e) => `Día ${e.day}`)
          .join(" → ")})`
      : "No definido";

  // ✨ MEJORA 8: Opciones totales
  const totalChoices = events.reduce(
    (sum, e) => sum + (e.choices ? e.choices.length : 0),
    0
  );
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" style="color: ${
          stats.unreachable > 0 ? "var(--error)" : "inherit"
        }">${stats.unreachable}</div>
      </div>
      <div class="stat-card">
...
function renderFlowchart() {
  ...
  const filteredNodes = flowchartData.nodes.filter((node) => {
    if (
      flowchartState.dayFilter !== "all" &&
      node.day !== parseInt(flowchartState.dayFilter)
    ) {
      return false;
    }
    if (
      flowchartState.typeFilter !== "all" &&
      node.type !== flowchartState.typeFilter
    ) {
      return false;
    }
    if (
      !flowchartState.showUnreachable &&
      flowchartData.unreachableNodes.has(node.id)
    ) {
      return false;
    }
    return true;
  ...
  if (filteredNodes.length === 0) {
    svg.innerHTML =
      '<text x="50%" y="50%" text-anchor="middle" fill="#666">No hay eventos que mostrar con los filtros actuales</text>';
    return;
  }
...
function drawEdge(svg, edge, positions) {
  ...
  line.setAttribute(
    "marker-end",
    edge.type === "lock" ? "url(#arrowhead-lock)" : "url(#arrowhead)"
  );
  ...
  const details = `
    <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 8px; margin-top: 1rem;">
      ...
      <p><strong>Texto:</strong> ${
        event.text ? event.text.substring(0, 100) + "..." : "Sin texto"
      }</p>
      <p><strong>Opciones:</strong> ${event.choices?.length || 0}</p>
      ${
        flowchartData.unreachableNodes.has(event.id)
          ? '<p style="color: var(--danger);"><strong><i data-lucide="triangle-alert"></i> Este evento es INALCANZABLE</strong></p>'
          : ""
      }
      <button class="btn-primary btn-small" onclick="editEvent(${
        node.index
      })"><i data-lucide="pencil"></i> Editar Evento</button>
    </div>
  `;
  ...
  const statsHTML = `
    ...
    <div style="margin-top: 1rem;">
      ...
      <ul>
        ${Object.entries(byDay)
          ...
          .map(
            ([day, count]) =>
              `<li><strong>Día ${day}:</strong> ${count} eventos</li>`
          )
          .join("")}
      </ul>
    </div>
    ...
    ${
      ...
      <div style="margin-top: 1rem;">
        ...
        <ul>
          ${Array.from(flowchartData.unreachableNodes)
            .map((id) => `<li>${id}</li>`)
            .join("")}
        </ul>
      </div>
...
window.applyFlowchartFilters = function () {
  flowchartState.dayFilter =
    document.getElementById("flowchartDayFilter").value;
  flowchartState.typeFilter = document.getElementById(
    "flowchartTypeFilter"
  ).value;
  flowchartState.showUnreachable =
    document.getElementById("showUnreachable").checked;
...
window.applyFlowchartZoom = function (value) {
  ...
  document.getElementById("flowchartZoomValue").textContent =
...
["storyDays", "startingDay", "saveSlots"].forEach((id) => {
  document.getElementById(id).addEventListener("input", (e) => {
    ...
    const target =
      id === "saveSlots"
        ? currentStory.config.settings
        : currentStory.config.story;
...
document
  .getElementById("defaultEndingMessage")
...
document.getElementById("saveBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(storyId)) {
      throw new Error(
        "El ID de la historia solo puede contener letras, números, guiones y guiones bajos"
      );
    }
...
document.getElementById("exportBtn").addEventListener("click", () => {
  try {
    ...
    if (!currentStory.config || !currentStory.story || !currentStory.endings) {
      throw new Error(
        "La historia no está completa. Verifica que tenga config, story y endings."
      );
    }
    ...
    const filename = `${
      currentStory.config.story.id || "historia"
    }_completa.json`;
...
document.getElementById("importFile").addEventListener("change", async (e) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "El archivo no contiene JSON válido: " + parseError.message
      );
    }
    ...
    if (!imported) {
      throw new Error(
        "Formato de archivo no reconocido. Debe ser un bundle completo o un archivo config/story/endings válido."
      );
    }
...
document.getElementById("validateBtn").addEventListener("click", () => {
  ...
  if (issues.length === 0) {
    results.innerHTML =
      '<div style="color: var(--success); padding: 2rem; text-align: center;"><h2>✅ Historia válida</h2><p>No se encontraron problemas</p></div>';
  } else {
    results.innerHTML = `
      <div style="color: var(--danger); padding: 1rem;">
        ...
        <ul style="margin-top: 1rem; padding-left: 2rem;">
          ${issues
            .map((issue) => `<li style="margin-bottom: 0.5rem;">${issue}</li>`)
            .join("")}
        </ul>
      </div>
    `;
